...
## Get a Filtered List of Blueprints

You can get a filtered list of blueprints based on their labels and the attribute `created_by`.
...
E.g. filtering by the following filter rules, will return all blueprints that their creator name starts with "alice" or "bob", 
...
$ curl -X POST \
    ...
    "http://<manager-ip>/api/v3.1/searches/blueprints?_include=id"
```
...
# Using requests
url = 'http://<manager-ip>/api/v3.1/searches/blueprints'
headers = {'Tenant': '<manager-tenant>'}
querystring = {'_include': 'id'}
data = {'filter_rules': [...]}
...
`POST "{manager-ip}/api/v3.1/searches/blueprints"`
...
Field | Type | Description
--------- | ------- | -------
`items` | list | A list of filtered `Blueprint` resources.
